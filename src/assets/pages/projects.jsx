...
function Home({ onScrollDownComplete, onScrollUpComplete, isActive = true }) {
  ...
  const triggerSkyExitRef = useRef(() => {})
  ...
  useEffect(() => {
    triggerSkyExitRef.current = () => {
      if (scrollUpPendingRef.current || scrollDownPendingRef.current) return
      if (viewStateRef.current === 'up') return
      if (cameraDropRef.current.active) return
      scrollUpPendingRef.current = true
      viewStateRef.current = 'up'
      swarmTriggerRef.current()
    }
    ...
    triggerFallExitRef.current = () => {
      if (scrollDownPendingRef.current || scrollUpPendingRef.current) return
